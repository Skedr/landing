        ...
        <div
          ...
          <div v-for="tier in tiers" :key="tier.id"
            ...
            <div class="p-8 lg:pt-12 xl:p-10 xl:pt-14">
              <h3 :id="tier.id"
                :class="[tier.featured ? 'text-gray-900' : 'text-white', 'text-sm font-semibold leading-6']">{{
              tier.name }}</h3>
              ...
              tier.price[frequency.value] }}</p>
                ...
                <a :href="tier.href" :aria-describedby="tier.id"
                  :class="[tier.featured ? 'bg-indigo-600 shadow-sm hover:bg-indigo-500 focus-visible:outline-indigo-600' : 'bg-white/10 hover:bg-white/20 focus-visible:outline-white', 'rounded-md py-2 px-3 text-center text-sm font-semibold leading-6 text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2']">Buy
                  this plan</a>
              </div>
              <div class="mt-8 flow-root sm:mt-10">
                <ul role="list"
                  :class="[tier.featured ? 'divide-gray-900/5 border-gray-900/5 text-gray-600' : 'divide-white/5 border-white/5 text-white', '-my-2 divide-y border-t text-sm leading-6 lg:border-t-0']">
          ...
          <div class="mx-auto max-w-2xl space-y-16">
            <div v-for="tier in tiers" :key="tier.id" class="border-t border-gray-900/10">
              <div
                ...
                <h3 :class="[tier.featured ? 'text-indigo-600' : 'text-gray-900', 'text-sm font-semibold leading-6']">{{
              tier.name }}</h3>
                      ...
                      <dl class="divide-y divide-gray-200 text-sm leading-6">
                        <div v-for="feature in section.features" :key="feature.name"
                          ...
                          <dd class="flex items-center justify-end sm:justify-center sm:px-4">
                            <span v-if="typeof feature.tiers[tier.id] === 'string'"
                              :class="tier.featured ? 'font-semibold text-indigo-600' : 'text-gray-900'">{{
              feature.tiers[tier.id] }}</span>
                            <template v-else>
                              <CheckIcon v-if="feature.tiers[tier.id] === true" class="mx-auto h-5 w-5 text-indigo-600"
                                aria-hidden="true" />
                              <XMarkIcon v-else class="mx-auto h-5 w-5 text-gray-400" aria-hidden="true" />
                              <span class="sr-only">{{ feature.tiers[tier.id] === true ? 'Yes' : 'No' }}</span>
                            </template>
                          </dd>
                        </div>
                      </dl>
          ...
          <div class="grid grid-cols-4 gap-x-8 border-t border-gray-900/10 before:block">
            <div v-for="tier in tiers" :key="tier.id" aria-hidden="true" class="-mt-px">
              <div :class="[tier.featured ? 'border-indigo-600' : 'border-transparent', 'border-t-2 pt-10']">
                <p :class="[tier.featured ? 'text-indigo-600' : 'text-gray-900', 'text-sm font-semibold leading-6']">{{
              tier.name }}</p>
                <p class="mt-1 text-sm leading-6 text-gray-600">{{ tier.description }}</p>
              </div>
            </div>
